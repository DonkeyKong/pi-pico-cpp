...
public:
  ...
  // Attempt to write specified vector, blocking until the specified absolute time is reached.
  int write_blocking_until(uint8_t addr, const std::vector<uint8_t>& src, bool nostop, absolute_time_t until)
  {
    return i2c_write_blocking_until(i2c_, addr, src.data(), src.size(), nostop, until);
  }
  ...
  int write_blocking(uint8_t addr, const std::vector<uint8_t>& src, bool nostop)
  {
    return i2c_write_blocking(i2c_, addr, src.data(), src.size(), nostop);
  }
