...
private:
  ...
  template <typename... T>
  static void setDutyCycle(float dutyCycle, T&... reg)
  {
    uint16_t onTime = 0;
  ...
  template <typename... T>
  static void setDutyCycle(bool on, T&... reg)
  {
  ...
  template <typename... T>
  static void setDutyCycle(uint16_t dutyCycle, T&... reg)
  {
    uint16_t onTime = 0;
...
public:
  ...
  void setSpeed(float power) override
  {
    ...
    if (power < 0.0f)
    {
      MotorKit::setDutyCycle(false, forward_);
      MotorKit::setDutyCycle(std::abs(power), backward_);
    }
    else if (power > 0.0f)
    {
      MotorKit::setDutyCycle(false, backward_);
      MotorKit::setDutyCycle(std::abs(power), forward_);
    }
    else
    {
      MotorKit::setDutyCycle(false, backward_, forward_);
    }
  }
  ...
  {
    powered_ = true;
    MotorKit::setDutyCycle(true, power_);
  }

  void powerOff()
  {
    powered_ = false;
    MotorKit::setDutyCycle(false, power_);
  }
private:
  bool powered_;
...
public:
  ...
  void updateCoils(int8_t pos, bool forceUpdateAll = false) override
  {
    ...
    if (wholeStepTransition) MotorKit::setDutyCycle(false, *offA, *offB);
    MotorKit::setDutyCycle(powerBegin, *begin);
    MotorKit::setDutyCycle(powerEnd, *end);
  }

  void grab() override
  {
    MotorKit::setDutyCycle(true, dutyCycleA_, dutyCycleB_);
    grabbed_ = true;
  }
  ...
  {
    grabbed_ = false;
    MotorKit::setDutyCycle(false, coil1A_, coil2A_, coil1B_, coil2B_, dutyCycleA_, dutyCycleB_);
  }
